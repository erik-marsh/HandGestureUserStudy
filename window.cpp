...
void SampleListener::onFrame(const Leap::Controller& controller)
{
    ...
    return;
    {
        ...
        //               << ", position: " << tool.tipPosition() << ", direction: " <<
        //               tool.direction()
        //               << std::endl;
        // }
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (frameCounter == 0) frameString = StringifyFrame(leapFrame);
        ...
        BeginDrawing();
        {
            ...
            BeginMode3D(camera);
            {
                ...
                for (auto it = hands.begin(); it != hands.end(); it++)
                {
                    ...
                    DrawLine3D(Debug::VECTOR_ORIGIN, Helpers::Vec3LeapToRaylib(state.palmNormal), BLACK);
                    Debug::DrawVectorDecomposition(Helpers::Vec3LeapToRaylib(state.palmNormal), true);

                    Input::ProcessedHandState processed = Input::ProcessHandState(state);

                    std::cout << "Cursor direction: (" << processed.cursorDirectionX << ", " << processed.cursorDirectionY << "); ";
                    std::cout << "Clicked?: " << processed.isInClickPose << std::endl;
                }
            }
            EndMode3D();
